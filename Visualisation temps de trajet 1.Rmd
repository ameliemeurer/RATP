...
```{r}
trajet_plus_court <- shortest_paths(temps_trajet_igraph, V(temps_trajet_igraph)[stop_name =="Pont de Neuilly"], to = V(temps_trajet_igraph)[stop_name =="République"], output = "vpath")

```

```{r}
df_trajet_plus_court
```
...
```{r}
df_distances <- as_data_frame(distances(temps_trajet_igraph, V(temps_trajet_igraph)[stop_name =="Pont de Neuilly"], to = V(temps_trajet_igraph)[stop_name =="République"]))
...
```
```{r}
V(temps_trajet_igraph)[trajet_plus_court$vpath[[1]]]$name
```
